...
# Load data
exp4.data <- read.csv("exp4_chain_complete.csv")
str(exp4.data)
writeLines("\n")
...
gm.pars <- fit.model(df=exp4.data,
...
gm.corr <- avg.model.data.corr(df=exp4.data, 
...
# Fit piecemeal model
pm.pars <- fit.model(df=exp4.data,
...
pm.corr = avg.model.data.corr(df=exp4.data, 
